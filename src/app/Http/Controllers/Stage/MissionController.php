...
{
    protected MissionRepository $repo;
    ...
    public function show(int $mission_code, int $group_id = null)
    {
        $missionEnum = MissionEnum::fromValue($mission_code);
        $params = $this->repo->{$missionEnum->key}($mission_code, $group_id);
        $params['missionEnum'] = $missionEnum;

        if ($group_id !== null && !isset($params['group']->id)) {
            return redirect('/groups');
        }

        return view('stage.missions.' . $missionEnum->blade())
            ->with($params);
    }
